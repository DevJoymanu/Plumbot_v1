...
- Each tenant picks the inbox its own internal alerts go to on the **Profile page** (`notification_email` → `TenantProfile.email_sender`). `get_plumber_notification_emails(tenant)` returns `PLATFORM_NOTIFICATION_EMAIL` (jones86xi@gmail.com — constant for every tenant) plus that address; with no choice made, only `tenant=None` and the homebase seed fall back to `settings.PLUMBER_NOTIFICATION_EMAILS`, so a new tenant never mails Homebase's inbox. Any new plumber-alert send path must pass the tenant.
