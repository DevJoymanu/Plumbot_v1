...
- **A contextual response outranks every template (owner rule, stated 2026-09-21 and again 2026-09-22).** Before any predetermined message goes out, a script, a nudge or a delay-flow message, it must fit what the lead just said and where the conversation is; if it does not, answer them instead. A scripted message is never sent again just because the flow is in a loop: a repeat goes through the model reader (`finalise_outbound` turns the check on for a draft that `repeats_recent_reply`), and scripted pushes stand aside for a lead who has deferred. [conversation-rules](docs/current-state/conversation-rules.md)
- **A lead who gets the plumber handoff gets TWO follow-ups, not four, and the handoff is the LAST text** until they reply: nothing from any loop (follow-ups, delay or parked nudges, the job-date ladder, check-backs) goes after it (`handoff_sent_since_last_reply`, `HANDOFF_TOUCHES`). A reply from the lead resets the count. [followups-and-cron](docs/current-state/followups-and-cron.md)
