...
### Email
- **Inbound: the bot answers email too** (`process_inbound_emails`, 5-min cron). A reply on a thread we started is matched by the `<apt-{id}.…>` Message-ID (or a legacy `[APT-{id}]` subject tag) and runs the full Hormozi reply path. An email with **no thread reference** is no longer skipped: it matches an existing lead by `customer_email`, else creates an **email lead** — synthetic `phone_number` of `email_<sha1[:12]>` (deterministic, so one sender never yields two rows), `lead_source='email'`, subject folded into the customer's turn because on a cold email the subject often IS the request. **Three gates keep the bot off everything in that mailbox that is not a customer** (fixed 2026-08-23 — the polled inbox was the operator's personal Gmail, so the bot opened leads on Stripe receipts and support tickets and replied to them): (1) `_is_our_own_mail` drops our own traffic BEFORE the thread tag is trusted — the polled address, `DEFAULT_FROM_EMAIL`, every tenant `customer_from_email`/`email_sender`, `PLATFORM_NOTIFICATION_EMAIL`, `PLUMBER_NOTIFICATION_EMAILS`, the owner accounts, and anything on `PLATFORM_EMAIL_DOMAIN`; the Bcc copy of our own customer email carries the same `[APT-{id}]` reference the customer's reply does, so an APT match alone is not proof a customer wrote. (2) A **cold** email is only answered when it was addressed (To / Cc / Delivered-To / X-Original-To …) to a **lead inbox**: `INBOUND_LEAD_ADDRESSES` (comma-separated) if set, else the polled mailbox itself — **unless that mailbox is one of our own inboxes, in which case cold intake is off entirely**, because nothing arriving in a personal/alert mailbox distinguishes a lead from a receipt. Thread replies are unaffected. (3) `_is_automated` drops machine mail (Auto-Submitted / List-Id / X-Autoreply headers, no-reply & mailer-daemon & receipt/billing/stripe addresses, bounce/out-of-office/receipt subjects) — without that guard the bot email-loops. Mail refused by gate 1 or 2 is left **UNREAD** and its body is never downloaded: fetches use `BODY.PEEK` (`_fetch_unseen_headers` → gates → `_fetch_message`), because a plain `RFC822` fetch silently marks the operator's own mail read. Any new mailbox-touching code must keep the peek-then-gate order. **Cold email leads are routed per tenant by the address they were delivered to** — the email equivalent of a WhatsApp `phone_number_id`, and the only routing key an inbound email carries. Each tenant forwards its own business address into the polled mailbox and `INBOUND_LEAD_ADDRESSES` maps it: `info@homebaseplumbers.co.zw=homebase,quotes@acmeplumbing.co.zw=acme`. `_lead_inbox_map` → `_lead_inbox_match` (a tenant-mapped address outranks a bare one on the same mail) → `_tenant_for_slug`; the lead is created AND looked up on that tenant (`_lead_for_sender` uses `for_tenant_or_seed` + `get_or_create_lead`, so the same person writing to two tenants gets two independent leads). A bare address, or the polled-mailbox fallback, still opens on the default tenant. **An unknown slug is never downgraded to the default tenant** — that would answer another company's customer with Homebase's prices, name and sender — it is logged as a TENANT ROUTE MISS and left unread so it retries once the env var is fixed. Thread replies are unaffected: they already carry `apt.tenant`. Note a non-homebase tenant also needs its `email_sending_enabled` flag ON (platform console) before any of its email actually goes out.
