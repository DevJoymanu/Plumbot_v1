        ...
        * 'I need a site visit' = same
        ...
        def _handle_service_inquiry_impl(self, intent, message):
                ...
                try:
                    ...
                    #
                    structured_pricing = build_structured_pricing(self.tenant_cfg)
                    # (Phase 2.3c: the bilingual per-intent price blocks render
                    # from TenantPriceItem rows — bot/pricing_copy.py. Homebase
                    # output is byte-identical to the old hardcoded dict, pinned
                    # by TenantConfigTests.test_structured_pricing_render_pinned.
                    # Intents missing from the tenant's sheet are absent here →
                    # the handler falls back to the free-visit deflect.)
        ...
        def _advance_after_scope(self, language: str = "english"):
            ...
            the next booking field using the EXACT approved first-pass script (never
            price the items). Returns None (fall through) when the next field isn't
            one we short-circuit for."""
            nq = self.get_next_question_to_ask()
            if nq in ('area', 'availability_date', 'availability_time'):
                scripted = self._get_first_pass_question(nq)
                if scripted:
                    self._set_question_retry_count(nq, 1)
                    return scripted
            if nq == 'name':
                return (
                    "One last thing — what name should we put on the booking? "
